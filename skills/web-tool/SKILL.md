---
name: web-tool
description: エルダーサイン向けWebツールの新規作成/編集/整理を行うためのスキル。`docs/web/` 配下のファイルを変更するときは必ずこのスキルを使う。`docs/web/` 配下のHTML作成・修正、必要に応じた `docs/*.js` の追加・更新、`docs/index.html` へのリンク追加、`docs/theme.css` との整合が必要な作業で使う。
---
...
`docs/web/` 配下のファイルを変更する依頼では、作業の規模にかかわらず必ずこのスキルを適用する。
...
3. 原則として1つのHTMLページに対して1つの同名JSを用意し、`docs/web/<name>.html` なら `docs/web/<name>.js` を対応させる。ページ固有の処理はまず同名JSに置き、共通化が必要なものだけを別ファイルへ切り出す。
4. 共有機能が必要なら `docs/firebase.js` をES moduleとして読み込む。
5. `docs/index.html` にツールへのリンクカードを追加・更新する。
6. 必要に応じて `docs/image/` にスクリーンショットを追加する。
7. HTMLから参照している `.css` や `.js` を実際に更新した場合は、コード修正の後で参照元HTMLに書かれている `?v1` のようなクエリ末尾を必ずインクリメントする。
8. 手順7は新規作成時だけでなく、既存ページの修正・リファクタリング時にも必ず適用する。HTML本体を触っていなくても、参照先のCSS/JSを更新したならHTML側のバージョン表記も更新する。
9. 作業完了前に、変更した外部ファイル参照（`theme.css` / `*.js` / `*.css`）のクエリバージョンを対象HTML全体で再確認する。
10. 対象ページで手動確認する。

### バージョン確認チェック（必須）

- 変更した外部ファイルがある場合は、公開対象HTML（`docs/web/**/*.html`）で当該参照を検索し、古い `?v` が残っていないことを確認する。
- `docs/web/*.html` から参照している `docs/web/*.js` / `docs/*.js` / `docs/*.css` を更新した場合は、更新したファイルごとに参照元HTMLの `?v` を 1 つ以上進める。
- 新規ページでは `docs/web/<name>.html` と `docs/web/<name>.js` の対応を先に確認し、その上で必要な共通JSだけを追加参照する。
...
- UIは既存のスタイルに合わせ、ページ破壊を避ける配置と `z-index` を意識する。
- 計算ツール（入力と出力を持つページ）は、出力エリアをページ上部に配置し、`position: sticky` で固定する。
- 数字のテキストフィールドを作成する際、特に指定がない場合は最低値と最大値を設定せず、自由に入力できるようにする。
